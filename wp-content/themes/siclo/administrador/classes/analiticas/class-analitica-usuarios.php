...
class Analitica_usuarios extends Analitica{
	/*
	...
	*/
	...
	private function crear_grafica( $tipo = 'usuarios_por_fechas', $data = false ){
		...
		$html.= '<figure class="grafica grafica_'.$tipo.'">';
	...
	private function grafica_usuarios_por_clases( $data = false, $porcentajes = false ){
		if( !$data ){ return; };
		$html = !$porcentajes ? 
			'<h2 class="grafica-titulo">Usuarios por clases</h2>' : 
			'<h2 class="grafica-titulo">Porcentaje asistencia</h2>';
		$html.= '<div class="grafica-cuerpo">';
				...
				$html.= '<div class="grafica-columna">';
					foreach( $info as $k => $info_este ){
						...
						if( !$porcentajes ){
							...
							$porcentaje	= (int)( ($info_este['asistencia']*100)/$maximo );
							$html.= 
