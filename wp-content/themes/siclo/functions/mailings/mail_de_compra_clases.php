<?php
function mail_de_compra_clases( $idPedido = false ){
	if( !$idPedido ){ return; };
	
	$pedido = new Pedido( $idPedido );
	$paqueteID = $pedido->paquete;	
	$paquete = $paqueteID == 'regalo' ? new Paquete_regalo($paqueteID, true) : new Paquete($paqueteID);
	$clases = $paquete->data['cantidad'];
	
	$p = get_post($idPedido);
	$idAuthor = $p->post_author;
	$user = get_user_by('id', $idAuthor);
	$name = $user->display_name;
	
	$html = 
		'<div style="max-width: 950px; background-color: rgb(255, 147, 30); margin: auto; box-sizing: border-box; padding: 28px 0; font-family: Helvetica;">'.
			...
			'<div style="width: 640px; background-color: #fff; padding: 30px 0 0; margin: auto;">'.
				...
				'<div style="display: block; vertical-align: top; max-width: 415px; padding: 0 30px; box-sizing: border-box; margin: auto; text-align: center;">'.
					'<p style="font-size: 27px; line-height: 30px; font-weight: 300; font-family: Helvetica; border-bottom: 3px solid rgb(255, 147, 30); padding-bottom: 25px; width: 100%; margin: auto; color: #535151;">Hola '. $name .',</p>'.
					...
					'<div style="font-size: 16px; color: #535151; font-weight: 800; padding: 20px 0; border-top: 1px solid #939598;">'.
						...
						'<span style="display: inline-block; margin: 0 5px;">'. $clases .' clases</span>'.
					'</div>'.
				...
				'<div style="display: block; vertical-align: top; max-width: 600px; margin: 30px auto; padding: 0 30px; box-sizing: border-box; border: 1px solid #dedede; box-shadow: -1px 0 1px 0 #dedede;">'.					
					...
					'<p style="font-size: 12px; font-family: Helvetica; text-align: justify; color: #939598;">No te preocupes, si tuviste un contratiempo podrás cancelar la clase sin ningún cargo siempre y cuando sea con al menos una hora de anticipación.</p>'.
				'</div>'.
	...
	wp_mail($mail, 'Has comprado una clase', $html, $headers);
