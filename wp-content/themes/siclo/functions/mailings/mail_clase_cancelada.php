<?php
function mail_clase_cancelada( $data_to_mail = false, $user = false ){
	if( !$data_to_mail || !$user ){ return; };
	/*$user es un objeto usuario*/
	/*$data_to_mail es un objeto reserva*/
	$name = $user->display_name;
	
	$html = 
		'<div style="width: 950px; background-color:  #ff00b7; margin: auto; box-sizing: border-box; padding: 28px 0;">'.
			...
			'<div style="width: 640px; background-color: #fff; padding: 30px 0; margin: auto; position: relative;">'.
				...
				'<p style="text-transform: uppercase; color: #dedede; text-align: center; font-family: Helvetica; font-size: 13px; margin-bottom: 50px;">clase cancelada</p>'.
				'<p style="font-size: 42px; text-align: center; font-weight: 300; font-family: Helvetica; border-bottom: 3px solid #ff00b7; padding-bottom: 45px; width: 490px; margin: auto; color: #535151; line-height: 50px;">Hola '. $name .',</p>'.
				'<p style="font-size: 24px; line-height: 30px; font-family: Helvetica; font-weight: 300; text-align: center; color: #414040; width: 490px; margin:  70px auto;">Te informamos que tu clase fue cancelada exitosamente. Esperamos que ruedes pronto con nosotros.</p>'.
	...
	wp_mail($mail, 'Tu clase ha sido cancelada', $html, $headers);
